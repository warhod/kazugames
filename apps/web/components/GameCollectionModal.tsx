...
export default function GameCollectionModal({
  ...
  useEffect(() => {
    ...
    fetch(`/api/collection/lookup?game_id=${encodeURIComponent(resolvedGameId)}`)
      .then(async (res) => {
        if (cancelled) return;
        if (res.status === 401) {
          setCollectionEntry(null);
  ...
  const addToCollection = async () => {
    ...
    try {
      const res = await fetch("/api/collection", {
        ...
        body: JSON.stringify({ game_id: resolvedGameId, status: selectedStatus }),
      });
      if (res.status === 401) {
        goToLogin();
        return;
      ...
      setCollectionEntry({ id: row.id, status: row.status, lendable: row.lendable });
  ...
  const updateStatus = async (next: GameStatus) => {
    ...
    try {
      ...
      setCollectionErr(e instanceof Error ? e.message : "Could not update status");
    } finally {
      setCollectionBusy(false);
    }
  };
  ...
  const removeFromCollection = async () => {
    ...
    try {
      ...
      setCollectionErr(e instanceof Error ? e.message : "Could not remove game");
    } finally {
      setCollectionBusy(false);
    }
  };
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`Manage collection for ${resolvedTitle}`}
    >
      <button
        type="button"
        ...
        <div className="md:flex">
          ...
          <div className="flex-1 p-5 md:p-7 flex flex-col gap-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[10px] font-display tracking-widest" style={{ color: "var(--accent)" }}>
                  QUICK ADD
                </p>
                <h2 className="text-xl font-display leading-tight" style={{ color: "var(--text-primary)" }}>
                  {resolvedTitle}
                </h2>
              ...
              <button type="button" onClick={onClose} className="btn-neon text-xs px-4 py-2">
                X
              </button>
            </div>
            ...
            <div className="text-sm" style={{ color: "var(--text-primary)" }}>
              {resolvedPrice !== null ? (
                <div className="flex items-center gap-2">
                  <span className="price-badge">${resolvedPrice.toFixed(2)}</span>
                  {resolvedMsrp && resolvedPrice < resolvedMsrp && (
                    <span className="price-msrp-strike">${resolvedMsrp.toFixed(2)}</span>
                  )}
                </div>
              ) : (
                <span style={{ color: "var(--text-muted)" }}>Availability unknown</span>
              )}
            </div>
            ...
            <div className="pt-3 border-t" style={{ borderColor: "var(--border-subtle)" }}>
              <p className="text-[10px] font-display tracking-[0.15em] uppercase mb-2" style={{ color: "var(--text-muted)" }}>
                CHOOSE A COLLECTION
              </p>
            ...
            <button type="button" onClick={onClose} className="btn-neon text-sm py-3 w-full">
              BACK TO RESULTS
            </button>
          </div>
        </div>
