"use client";

import { useEffect, useMemo, useState } from "react";
import { createPortal } from "react-dom";
import Link from "next/link";
import type {
  DbGroup,
  DbGameLoan,
  DbGroupMemberView,
  DbLendableGameRow,
  DbPublicProfile,
} from "@/lib/database.types";
import { buildGroupInviteShareUrl } from "@/lib/group-invite";
import useScrollLock from "@/lib/useScrollLock";
import GameCard from "./GameCard";

interface GroupDetail extends DbGroup {
  members: DbGroupMemberView[];
  lendable_games: DbLendableGameRow[];
}

interface GroupPanelProps {
  group: GroupDetail;
  currentUserId?: string;
  loans?: DbGameLoan[];
  onRequestLoan?: (
    gameId: string,
    ownerId: string,
    groupId: string,
    borrowerId?: string,
  ) => void;
  onAcceptLoan?: (loanId: string) => void;
  onDeleteGroup?: (groupId: string) => void;
  onRemoveMember?: (groupId: string, userId: string) => void;
}

type ConfirmAction =
  | { kind: "delete-group" }
  | { kind: "remove-member"; userId: string; memberName: string };

function shortUserId(userId: string) {
  return userId.slice(0, 8).toUpperCase();
}

function displayNameForMember(
  profile: DbPublicProfile,
  userId: string,
  currentUserId: string | undefined,
) {
  const name = profile.display_name?.trim();
  if (name) return name;
  if (userId === currentUserId) return "YOU";
  const hint = profile.account_hint?.trim();
  if (hint) return hint;
  return shortUserId(userId);
}

function ownerLabel(profile: DbPublicProfile, ownerUserId: string) {
  const name = profile.display_name?.trim();
  if (name) return name;
  const hint = profile.account_hint?.trim();
  if (hint) return hint;
  return shortUserId(ownerUserId);
}

function buildInviteShareClipboardText(
  groupName: string,
  groupId: string,
  inviteCode: string,
) {
  if (typeof window === "undefined") return "";
  const link = buildGroupInviteShareUrl(
    window.location.origin,
    groupId,
    inviteCode,
  );
  return `Join my group "${groupName}" on Kazu Games — open the link, create a free account (or sign in), then tap Join:\n\n${link}`;
}

function safeHttpsUrl(url: string | null): string | null {
  const u = url?.trim();
  if (!u) return null;
  try {
    const parsed = new URL(u);
    return parsed.protocol === "https:" ? parsed.href : null;
  } catch {
    return null;
  }
}

export default function GroupPanel({
  group,
  currentUserId,
  loans = [],
  onRequestLoan,
  onAcceptLoan,
  onDeleteGroup,
  onRemoveMember,
}: GroupPanelProps) {
  const [showInviteModal, setShowInviteModal] = useState(false);
  const [inviteModalCopied, setInviteModalCopied] = useState(false);
  const [copiedFriendFor, setCopiedFriendFor] = useState<string | null>(null);
  const [confirmAction, setConfirmAction] = useState<ConfirmAction | null>(
    null,
  );
  const [lendModalGame, setLendModalGame] = useState<DbLendableGameRow | null>(
    null,
  );
  const [selectedBorrowerId, setSelectedBorrowerId] = useState<string>("");
  useScrollLock(
    showInviteModal || Boolean(lendModalGame) || confirmAction !== null,
  );
  const isOwner = currentUserId === group.owner_id;

  const inviteShareText = useMemo(() => {
    if (typeof window === "undefined" || !showInviteModal) return "";
    return buildInviteShareClipboardText(
      group.name,
      group.id,
      group.invite_code,
    );
  }, [showInviteModal, group.name, group.id, group.invite_code]);

  const openInviteModal = () => {
    setInviteModalCopied(false);
    setShowInviteModal(true);
  };

  const closeInviteModal = () => {
    setShowInviteModal(false);
    setInviteModalCopied(false);
  };

  const copyInviteFromModal = async () => {
    const text = buildInviteShareClipboardText(
      group.name,
      group.id,
      group.invite_code,
    );
    try {
      await navigator.clipboard.writeText(text);
      setInviteModalCopied(true);
      setTimeout(() => setInviteModalCopied(false), 2800);
    } catch {
      setInviteModalCopied(false);
    }
  };

  useEffect(() => {
    if (!showInviteModal) return;
    const onEsc = (e: KeyboardEvent) => {
      if (e.key === "Escape") closeInviteModal();
    };
    window.addEventListener("keydown", onEsc);
    return () => window.removeEventListener("keydown", onEsc);
  }, [showInviteModal]);

  const copyFriendCode = async (userId: string, code: string) => {
    try {
      await navigator.clipboard.writeText(code);
      setCopiedFriendFor(userId);
      setTimeout(() => setCopiedFriendFor(null), 2000);
    } catch {
      // ignore
    }
  };

  const hasActiveLoan = (gameId: string, ownerId: string) => {
    return loans.some(
      (l) =>
        l.game_id === gameId &&
        l.owner_id === ownerId &&
        (l.status === "requested" || l.status === "approved"),
    );
  };

  const approvedLoanFor = (gameId: string, ownerId: string) => {
    return loans.find(
      (l) =>
        l.game_id === gameId &&
        l.owner_id === ownerId &&
        l.status === "approved",
    );
  };

  const pendingRequestsForMe = loans.filter((loan) => {
    if (
      loan.group_id !== group.id ||
      loan.status !== "requested" ||
      !currentUserId
    ) {
      return false;
    }
    const requesterId = loan.requested_by_id ?? loan.borrower_id;
    const requesteeId =
      requesterId === loan.owner_id ? loan.borrower_id : loan.owner_id;
    return requesteeId === currentUserId;
  });

  const openLendModal = (game: DbLendableGameRow) => {
    setLendModalGame(game);
    setSelectedBorrowerId("");
  };

  const closeLendModal = () => {
    setLendModalGame(null);
    setSelectedBorrowerId("");
  };

  const closeConfirmModal = () => {
    setConfirmAction(null);
  };

  const runConfirmedAction = () => {
    if (!confirmAction) return;
    if (confirmAction.kind === "delete-group") {
      onDeleteGroup?.(group.id);
    } else if (confirmAction.kind === "remove-member") {
      onRemoveMember?.(group.id, confirmAction.userId);
    }
    closeConfirmModal();
  };

  const memberNameById = new Map(
    group.members.map((m) => [
      m.user_id,
      displayNameForMember(m.profile, m.user_id, currentUserId),
    ]),
  );

  const selectableBorrowers =
    lendModalGame && currentUserId
      ? group.members.filter((m) => m.user_id !== lendModalGame.user_id)
      : [];

  return (
    <div className="glass-card overflow-hidden">
      {/* Header */}
      <div
        className="p-5 border-b flex items-start justify-between gap-4"
        style={{ borderColor: "var(--border-subtle)" }}
      >
        <div className="min-w-0">
          <h3
            className="font-display text-lg tracking-widest uppercase truncate"
            style={{ color: "var(--accent)" }}
          >
            {group.name}
          </h3>
          <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
            {group.members.length} member{group.members.length !== 1 ? "s" : ""}{" "}
            &middot; Created {new Date(group.created_at).toLocaleDateString()}
          </p>
        </div>

        <div className="flex items-center gap-2 shrink-0">
          <button
            type="button"
            onClick={openInviteModal}
            className="btn-neon btn-neon-cyan text-[10px] px-3 py-1.5 max-w-[11rem] leading-tight whitespace-normal text-center"
            title="Open invite message to copy and share with friends"
          >
            INVITE
          </button>
          {isOwner && onDeleteGroup && (
            <button
              onClick={() => setConfirmAction({ kind: "delete-group" })}
              className="btn-neon btn-neon-pink text-[10px] px-3 py-1.5"
              title="Delete group"
            >
              DELETE
            </button>
          )}
        </div>
      </div>

      {/* Loan requests (prioritized at top of group content) */}
      <div className="px-5 py-4">
        {pendingRequestsForMe.length > 0 && (
          <div
            className="rounded-lg border p-3"
            style={{
              borderColor: "var(--border-subtle)",
              background: "var(--bg-elevated)",
            }}
          >
            <h4
              className="text-[10px] font-display tracking-[0.2em] uppercase mb-2"
              style={{ color: "var(--text-muted)" }}
            >
              REQUESTS FOR YOU ({pendingRequestsForMe.length})
            </h4>
            <ul className="space-y-2">
              {pendingRequestsForMe.map((loan) => {
                const requesterId = loan.requested_by_id ?? loan.borrower_id;
                const requester =
                  memberNameById.get(requesterId) ?? shortUserId(requesterId);
                const title = loan.game?.title ?? "Unknown game";
                return (
                  <li
                    key={loan.id}
                    className="flex flex-wrap items-center justify-between gap-2 rounded-md border px-2.5 py-2"
                    style={{ borderColor: "var(--border-subtle)" }}
                  >
                    <p
                      className="text-xs"
                      style={{ color: "var(--text-primary)" }}
                    >
                      <span style={{ color: "var(--text-muted)" }}>
                        {requester}
                      </span>{" "}
                      requested <strong>{title}</strong>
                    </p>
                    {onAcceptLoan && (
                      <button
                        type="button"
                        onClick={() => onAcceptLoan(loan.id)}
                        className="btn-neon btn-neon-solid text-[10px] py-1.5 px-3"
                      >
                        ACCEPT
                      </button>
                    )}
                  </li>
                );
              })}
            </ul>
          </div>
        )}
      </div>

      {/* Members */}
      <div
        className="px-5 py-4 border-t border-b"
        style={{ borderColor: "var(--border-subtle)" }}
      >
        <h4
          className="text-[10px] font-display tracking-[0.2em] uppercase mb-3"
          style={{ color: "var(--text-muted)" }}
        >
          MEMBERS
        </h4>
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {group.members.map((member) => {
            const isSelf = member.user_id === currentUserId;
            const isMemberOwner = member.user_id === group.owner_id;
            const primary = displayNameForMember(
              member.profile,
              member.user_id,
              currentUserId,
            );
            const friend = member.profile.friend_code?.trim() || null;
            const nintendoUrl = safeHttpsUrl(
              member.profile.nintendo_profile_url,
            );

            return (
              <li
                key={member.user_id}
                className="flex items-start gap-3 rounded-lg border px-3 py-2.5"
                style={{
                  borderColor: isMemberOwner
                    ? "var(--accent)"
                    : "var(--border-subtle)",
                  background: "var(--bg-elevated)",
                }}
              >
                <span
                  className="mt-0.5 w-7 h-7 rounded-full flex shrink-0 items-center justify-center text-xs font-bold"
                  style={{
                    background: isMemberOwner
                      ? "color-mix(in srgb, var(--accent) 22%, transparent)"
                      : "var(--bg-surface)",
                    color: isMemberOwner
                      ? "var(--accent)"
                      : "var(--text-muted)",
                  }}
                  aria-hidden
                >
                  {isMemberOwner ? "★" : "●"}
                </span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex flex-wrap items-center gap-2">
                      <Link
                        href={`/profile/${encodeURIComponent(member.user_id)}`}
                        className="font-display text-xs tracking-wide rounded-sm underline-offset-2 hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
                        style={{
                          color: "var(--text-primary)",
                          outlineColor: "var(--accent)",
                        }}
                        aria-label={`View ${primary === "YOU" ? "your" : `${primary}'s`} profile`}
                      >
                        {primary}
                      </Link>
                      {isSelf && member.profile.display_name?.trim() && (
                        <span
                          className="text-[10px] uppercase tracking-wider"
                          style={{ color: "var(--text-muted)" }}
                        >
                          (you)
                        </span>
                      )}
                    </div>
                    {isOwner && !isSelf && onRemoveMember && (
                      <button
                        onClick={() =>
                          setConfirmAction({
                            kind: "remove-member",
                            userId: member.user_id,
                            memberName: primary,
                          })
                        }
                        className="shrink-0 w-6 h-6 flex items-center justify-center rounded-full text-[10px] hover:opacity-100 opacity-60 transition-opacity"
                        style={{ color: "var(--accent-secondary)" }}
                        title="Remove member"
                        type="button"
                      >
                        ✕
                      </button>
                    )}
                  </div>
                  <div
                    className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]"
                    style={{ color: "var(--text-muted)" }}
                  >
                    {friend && (
                      <span className="inline-flex items-center gap-1.5 font-mono">
                        {friend}
                        <button
                          type="button"
                          onClick={() => copyFriendCode(member.user_id, friend)}
                          className="uppercase tracking-wider underline-offset-2 hover:underline"
                          style={{ color: "var(--accent)" }}
                        >
                          {copiedFriendFor === member.user_id
                            ? "Copied"
                            : "Copy"}
                        </button>
                      </span>
                    )}
                    {nintendoUrl && (
                      <a
                        href={nintendoUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 uppercase tracking-wider"
                        style={{ color: "var(--accent-secondary)" }}
                      >
                        <span aria-hidden>↗</span> Nintendo
                      </a>
                    )}
                  </div>
                </div>
              </li>
            );
          })}
        </ul>
      </div>

      {/* Lendable games */}
      <div className="px-5 py-4">
        <h4
          className="text-[10px] font-display tracking-[0.2em] uppercase mb-3"
          style={{ color: "var(--text-muted)" }}
        >
          AVAILABLE TO BORROW ({group.lendable_games.length})
        </h4>

        {group.lendable_games.length === 0 ? (
          <p
            className="text-xs py-6 text-center"
            style={{ color: "var(--text-muted)" }}
          >
            {`No games available to borrow yet. Owned, completed, or dropped titles in a member's collection appear here for the group.`}
          </p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {group.lendable_games.map((ug) => {
              const alreadyLoaned = hasActiveLoan(ug.game_id, ug.user_id);
              const isSelf = ug.user_id === currentUserId;
              const approvedLoan = approvedLoanFor(ug.game_id, ug.user_id);
              const loanBadgeLabel = approvedLoan
                ? approvedLoan.borrower_id === currentUserId
                  ? "LENT"
                  : approvedLoan.owner_id === currentUserId
                    ? "LENT OUT"
                    : undefined
                : undefined;
              return (
                <div key={ug.id} className="flex flex-col gap-2 min-w-0">
                  <div
                    className="inline-flex items-center gap-1.5 self-start rounded-full border px-2.5 py-1 text-[10px] font-display tracking-wider uppercase"
                    style={{
                      borderColor: "var(--border-subtle)",
                      background:
                        "color-mix(in srgb, var(--accent) 12%, transparent)",
                      color: "var(--text-primary)",
                    }}
                  >
                    <span style={{ color: "var(--text-muted)" }}>Owner</span>
                    <span
                      className="font-semibold"
                      style={{ color: "var(--accent)" }}
                    >
                      {ownerLabel(ug.owner_profile, ug.user_id)}
                    </span>
                  </div>
                  <div className="relative min-w-0">
                    <GameCard
                      id={ug.game.id}
                      title={ug.game.title}
                      deku_url={ug.game.deku_url}
                      image_url={ug.game.image_url}
                      current_price={ug.game.current_price}
                      msrp={ug.game.msrp}
                      platform={ug.game.platform}
                      showPrices={false}
                      loanBadgeLabel={loanBadgeLabel}
                      primaryLinkLabel={
                        alreadyLoaned
                          ? "LOAN PENDING"
                          : isSelf
                            ? "LEND TO FRIEND"
                            : "REQUEST TO LEND"
                      }
                      onPrimaryAction={
                        onRequestLoan
                          ? () => {
                              if (isSelf) {
                                openLendModal(ug);
                              } else {
                                onRequestLoan(ug.game_id, ug.user_id, group.id);
                              }
                            }
                          : undefined
                      }
                      primaryActionDisabled={alreadyLoaned}
                    />
                  </div>
                </div>
              );
            })}
          </div>
        )}
      </div>

      {showInviteModal &&
        typeof document !== "undefined" &&
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <button
              type="button"
              className="absolute inset-0 bg-black/70"
              onClick={closeInviteModal}
              aria-label="Close invite modal"
            />
            <div
              className="relative w-full max-w-lg rounded-xl border p-5"
              style={{
                background: "var(--bg-surface)",
                borderColor: "var(--border-subtle)",
              }}
              role="dialog"
              aria-modal="true"
              aria-labelledby="group-invite-modal-title"
            >
              <h3
                id="group-invite-modal-title"
                className="font-display text-sm tracking-[0.15em] uppercase mb-2"
                style={{ color: "var(--accent)" }}
              >
                INVITE A FRIEND
              </h3>
              <p
                className="text-xs mb-3 leading-relaxed"
                style={{ color: "var(--text-muted)" }}
              >
                Copy the message below and send it to a friend. They can open
                the link, sign up or sign in, then join your group with this
                invite code.
              </p>
              <label htmlFor="group-invite-message" className="sr-only">
                Invite message to copy
              </label>
              <textarea
                id="group-invite-message"
                readOnly
                rows={6}
                value={inviteShareText}
                className="input-neon w-full text-xs mb-3 resize-y font-sans leading-relaxed min-h-[120px]"
                style={{ background: "var(--bg-elevated)" }}
                onFocus={(e) => e.currentTarget.select()}
              />
              <p
                className="text-md font-mono mb-4 leading-normal"
                style={{ color: "var(--text-muted)" }}
              >
                <span
                  className="font-display tracking-wider uppercase mr-1 text-xs"
                  style={{ color: "var(--text-primary)" }}
                >
                  Invite code:
                </span>{" "}
                <code
                  className="text-xs font-bold tracking-wider"
                  style={{ color: "var(--accent)" }}
                >
                  {group.invite_code}
                </code>
              </p>
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={closeInviteModal}
                  className="btn-neon text-xs flex-1 min-w-[6rem]"
                >
                  CLOSE
                </button>
                <button
                  type="button"
                  onClick={() => void copyInviteFromModal()}
                  className="btn-neon btn-neon-solid text-xs flex-1 min-w-[6rem]"
                >
                  {inviteModalCopied ? "COPIED" : "COPY MESSAGE"}
                </button>
              </div>
            </div>
          </div>,
          document.body,
        )}

      {lendModalGame &&
        onRequestLoan &&
        typeof document !== "undefined" &&
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <button
              type="button"
              className="absolute inset-0 bg-black/70"
              onClick={closeLendModal}
              aria-label="Close lend request modal"
            />
            <div
              className="relative w-full max-w-md rounded-xl border p-5"
              style={{
                background: "var(--bg-surface)",
                borderColor: "var(--border-subtle)",
              }}
            >
              <h3
                className="font-display text-sm tracking-[0.15em] uppercase mb-2"
                style={{ color: "var(--accent)" }}
              >
                LEND TO FRIEND
              </h3>
              <p
                className="text-xs mb-3"
                style={{ color: "var(--text-muted)" }}
              >
                Select who should receive this request for{" "}
                <strong style={{ color: "var(--text-primary)" }}>
                  {lendModalGame.game.title}
                </strong>
                .
              </p>
              <select
                value={selectedBorrowerId}
                onChange={(e) => setSelectedBorrowerId(e.target.value)}
                className="input-neon w-full text-xs mb-4"
              >
                <option value="" disabled>
                  Choose a friend
                </option>
                {selectableBorrowers.map((m) => (
                  <option key={m.user_id} value={m.user_id}>
                    {memberNameById.get(m.user_id) ?? shortUserId(m.user_id)}
                  </option>
                ))}
              </select>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={closeLendModal}
                  className="btn-neon text-xs flex-1"
                >
                  CANCEL
                </button>
                <button
                  type="button"
                  disabled={!selectedBorrowerId}
                  onClick={() => {
                    onRequestLoan(
                      lendModalGame.game_id,
                      lendModalGame.user_id,
                      group.id,
                      selectedBorrowerId,
                    );
                    closeLendModal();
                  }}
                  className="btn-neon btn-neon-solid text-xs flex-1 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  SEND REQUEST
                </button>
              </div>
            </div>
          </div>,
          document.body,
        )}

      {confirmAction &&
        typeof document !== "undefined" &&
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <button
              type="button"
              className="absolute inset-0 bg-black/70"
              onClick={closeConfirmModal}
              aria-label="Close confirmation modal"
            />
            <div
              className="relative w-full max-w-md rounded-xl border p-5"
              style={{
                background: "var(--bg-surface)",
                borderColor: "var(--border-subtle)",
              }}
            >
              <h3
                className="font-display text-sm tracking-[0.15em] uppercase mb-2"
                style={{ color: "var(--accent-secondary)" }}
              >
                CONFIRM ACTION
              </h3>
              <p
                className="text-sm mb-4"
                style={{ color: "var(--text-primary)" }}
              >
                {confirmAction.kind === "delete-group"
                  ? `Delete "${group.name}"? This removes all members and cannot be undone.`
                  : `Remove ${confirmAction.memberName} from "${group.name}"?`}
              </p>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={closeConfirmModal}
                  className="btn-neon text-xs flex-1"
                >
                  CANCEL
                </button>
                <button
                  type="button"
                  onClick={runConfirmedAction}
                  className="btn-neon btn-neon-pink text-xs flex-1"
                >
                  {confirmAction.kind === "delete-group"
                    ? "DELETE GROUP"
                    : "REMOVE MEMBER"}
                </button>
              </div>
            </div>
          </div>,
          document.body,
        )}
    </div>
  );
}
